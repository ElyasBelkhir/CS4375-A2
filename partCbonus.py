...
class FFNN(nn.Module):
    def __init__(self, input_dim, h):
        ...
        self.activation = nn.Sigmoid()  # The rectified linear unit; one valid choice of activation function
        self.output_dim = 3
        self.W2 = nn.Linear(h, self.output_dim)

        self.softmax = nn.LogSoftmax(
            dim=-1)  # The softmax function that converts vectors into probability distributions; computes log probabilities for computational benefits
...
if __name__ == "__main__":
    ...
    parser.add_argument("-hd", "--hidden_dim", type=int, nargs='+', required=True, help="hidden_dim")
    ...
    # load data
    print("========== Loading data ==========")
    train_data, valid_data, test_data = load_data(args.train_data, args.val_data,
                                                  args.test_data)  # X_data is a list of pairs (document, y); y in {0,1,2,3,4}
    ...
    for hidden_dim in args.hidden_dim:
        print(f"Training model with hidden dimension: {hidden_dim}")
        ...
        plt.xlabel('Epochs')
        plt.ylabel('Accuracy')
        plt.title(f'Training and Validation Accuracies for Hidden Dimension {hidden_dim}')
        plt.legend()
        plt.show()

        plt.plot(range(1, args.epochs + 1), f1_scores, label='F1 Score')
        plt.xlabel('Epochs')
        plt.ylabel('F1 Score')
        plt.title(f'F1 Score Over Epochs for Hidden Dimension {hidden_dim}')
        plt.legend()
        plt.show()
